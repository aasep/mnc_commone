...
<!-- BEGIN SAMPLE PORTLET CONFIGURATION MODAL FORM-->
 ...
 <?php
	...
	echo "<div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button><strong>Add memo failed ...!</strong> </div>";
...
?>
                        ...
                        <div class="portlet light bordered">
									<div class="portlet-title">
										<div class="caption">
											...
											<span class="caption-subject font-red-sunglo bold uppercase">Call Customer , if the problem has been Finished</span>
						...
						<div class="portlet-body form">
                            ...
                            $query .=" left join master_case b on b.id_case=a.id_case  where a.call_to_close='1' and a.status='3' and a.id_cabang='".getIdCabang()."' ";
                            $query .=" and a.id_channel='".getIdChannel()."' order by a.ticket_number asc ";
                            ...
                            while ($row=pg_fetch_array($result)) {
                             ...
                             switch ($row['priority']) {
                              ...
                              switch ($row['status']) {
		 				 		...
		 				 		case '2' : $status_lap= "  <span class='label label-info'>  OnProgress  </span> "; break; //customer_name
                        ...
                        <div class="portlet light bordered">
									<div class="portlet-title">
										<div class="caption">
											<i class="icon-equalizer font-red-sunglo"></i>
											<span class="caption-subject font-red-sunglo bold uppercase">Call Customer for Completing Information</span>
											<span class="caption-helper"> </span>
										</div>
										<div class="actions">
						...
						<div class="portlet-body form">
                            ...
                            $query  =" select *, b.name as nama_masalah, b.priority,b.satuan,c.name as nama_channel from laporan_kerja a ";
                            $query .=" left join master_channel c on c.id_channel=a.id_channel ";
                            $query .=" left join master_case b on b.id_case=a.id_case  where a.call_to_ask='1' and a.status in (1,2) and a.id_cabang='".getIdCabang()."' " ;
                            $query .=" and a.id_channel='".getIdChannel()."' order by a.ticket_number asc ";
							...
							echo "<td style='font-size:11px' align='center'> $status_lap <br><br> <b>$priority <b></td>";
							echo "<td style='font-size:12px' ><b>$jml_intval  $satuan <br><i class='fa fa-calendar'></i> ".$date_sla."</b></td>";
							// echo "<td style='font-size:11px' align='center'> <a href='#'  data-toggle='modal' id-call_to_ask='$row[call_to_ask]' id-ticket='$row[ticket_number]' id-process='$row[id_process]' id-nama='$row[name]'  nm-satuan='$row[satuan]' data-target='#view-detail' class='detailLaporan' > <button class='btn blue btn-xs' type='button'> <i class='fa fa-check-circle'></i> <b> Detail</b> </button> </a></td>";
							echo "<td style='font-size:11px' align='center'> <a href='$page&ext=detail&tn=$row[ticket_number]&cat=2&ds=$_GET[iDisplayStart]'  target='_blank' > <button class='btn blue' type='button'> <i class='fa fa-check-circle'></i> <b> Detail</b> </button> </a> <br> <br> <a href='module/convert_pdf_lapker.php?module=$module&pm=$pm&tn=$row[ticket_number]' target='_blank' > <button class='btn red ' type='button'> Print <span aria-hidden='true' class='icon-printer'></span> </button></a> </td>";
...
<div class="portlet light bordered">
                                            <div class="portlet-title">
                                                <div class="caption">
                                                    ...
                                                    <span class="caption-subject font-green-haze bold uppercase">FORM Call Customer </span>
                                                    <span class="caption-helper"> Confirmation Call Customer ...</span>
                                    ...
                                    <!-- BEGIN FORM-->
                                    <form action="<?php echo "module/actions_master.php?module=$module&pm=$pm&act=memo_call_customer&ds=$_GET[ds]"; ?>" id="form_sample_1" class="form-horizontal" method="POST" enctype="multipart/form-data">
 ...
 */
	$('.detailLaporan').click(function() {
		...
		if (call_to_close === null || !call_to_close) {
		if (call_to_ask =='0'){
			$(".call_to_close").empty();
			$(".call_to_ask").empty();
		...
		if (call_to_close =='1'){
			$(".call_to_ask").empty();
			$(".call_to_close").empty();
			$(".call_to_close").append( '<div class="alert alert-danger alert-dismissable"><blink> <center><b> Note : Call Customer , if the problem has been Finished....!</b> </center></blink></div>');
			$(".call_to_close").append( '<input type="checkbox" value="0" name="call_to_close"/> I have already called customer  ');
		...
		$.ajax({
		type: "POST",
		url: "module/ajax_call_to_ask.php",
		data: dataString2,
		cache: false,
		success: function(html)
		{
...
$('#submit_conf').click(function() {
	...
	//alert ('oke.....!');
	if(!document.getElementById('var_note').checked){
