...
if (isset($_GET['act1']) ) {
    ...
    switch ($_GET['act1']) {
         case '1' : $varsrc= " where a.status='1' and a.id_cabang='".getIdCabang()."' and a.id_channel='".getIdChannel()."' and  to_char(a.date_sla, 'YYYY-MM-DD') >='".$current_date."' $var_srcby  "; $ket="Unprocessed"; break; //ticket_number
         case '2' : $varsrc= " where a.status='2' and a.id_cabang='".getIdCabang()."' and a.id_channel='".getIdChannel()."' and  to_char(a.date_sla, 'YYYY-MM-DD') >='".$current_date."' $var_srcby "; $ket="OnProgress"; break; //customer_name
         case '3' : $varsrc= " where a.status='3' and a.id_cabang='".getIdCabang()."' and a.id_channel='".getIdChannel()."' $var_srcby "; $ket="Done"; break; //account_number
         case '4' : $varsrc= " where a.status in ('1','2') and a.id_cabang='".getIdCabang()."' and a.id_channel='".getIdChannel()."' and  to_char(a.date_sla, 'YYYY-MM-DD') < '".$current_date."' $var_srcby "; $ket=" Exipred "; break; // credit_card_number
         }
        ...
        for ( $i=0 ; $i<count($aColumns) ; $i++ )
        {
		 ...
		 //case '4' : $varsrc= " where credit_card_number='$srckey' "; break; // credit_card_number
			...
			{				
               	 ...
               	 $nama_action="<a href='$page&ext=detail&tn=$aRow[ticket_number]&cat=$_GET[act1]&ds=$_GET[iDisplayStart]' target='_blank' > <button class='btn blue' type='button'> <i class='fa fa-check-circle'></i> Detail </button></a> <br><br>";  
                 $nama_action.="<a href='module/convert_pdf_lapker.php?tn=$aRow[ticket_number]' target='_blank'> <button class='btn red ' type='button'> Print <span aria-hidden='true' class='icon-printer'></span> </button></a> ";  
			   $row[] = $nama_action; 
			}		
