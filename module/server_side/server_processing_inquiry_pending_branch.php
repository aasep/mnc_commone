...
if (isset($_GET['srcby']) && ($_GET['srcby'])!="" ) {
 ...
 $varsrc= " where a.status='4' and a.id_cabang='".getIdCabang()."' and a.id_channel='".getIdChannel()."' $var_srcby ";
    ...
    while ( $aRow = pg_fetch_array($rResult, null, PGSQL_ASSOC) )
    {
        ...
        for ( $i=0 ; $i<count($aColumns) ; $i++ )
        {
         ...
         //                      switch ($aRow['status']) {
			   ...
			   $nama_status="<p align='center'> <br><br> <b>$priority <b> </p>";							
			   $row[] = $nama_status;
			}
			...
			{				
			   // $nama_action="<a href='#'  data-toggle='modal' id-status='".$aRow['status']."' id-call_to_ask='".$aRow['call_to_ask']."' id-ticket='".$aRow['ticket_number']."' id-process='".$aRow['id_process']."' id-nama='".$aRow['name']."' nm-satuan='".$aRow['satuan']."'  data-target='#view-detail' class='detailLaporan' > <button class='btn blue btn-xs' type='button'> <i class='fa fa-check-circle'></i> Detail </button></a>";	
               	 $nama_action="<a href='$page&ext=detail&tn=$aRow[ticket_number]&cat=$_GET[act1]&ds=$_GET[iDisplayStart]' target='_blank' > <button class='btn blue' type='button'> <i class='fa fa-check-circle'></i> Detail </button></a>";  
