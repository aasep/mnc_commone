...
*/
    ...
    $aColumns = array('id_pnews','title' );
	$aColumns_sorting = array('id_pnews','title');

    /* Indexed column (used for fast and accurate table cardinality) */
    $sIndexColumn = "id_pnews";
...
*/
	...
	$sQuery  = " select a.id_pnews,a.title,a.news,a.image,a.adddt,a.addby,a.id_cat_pnews,a.status,b.name_category from product_news a 
left join category_productnews b on b.id_cat_pnews=a.id_cat_pnews $sWhere   ".
    // $searchticket.
	// $searchcustomer.
    $sOrder." ".
	$sLimit;
	...
	$sQuery  = " select a.$sIndexColumn from product_news a 
left join category_productnews b on b.id_cat_pnews=a.id_cat_pnews 
 $sWhere   ";
    //	$searchticket.
	//    $searchcustomer." " ;
    ...
    while ( $aRow = pg_fetch_array($rResult, null, PGSQL_ASSOC) )
    {
        ...
        for ( $i=0 ; $i<count($aColumns) ; $i++ )
        {
			...
			else if ( $aColumns[$i] == "id_pnews" )
            {

                    $filename='images/img-prodnews/$aRow[image]';
                    if (!file_exists($filename)) {
                       $ticket=" ";
                    }else{
                       $ticket="<img src='images/img-prodnews/$aRow[image]' alt='' class='img-responsive'>";
                    }
			...
			else if ($aColumns[$i] == "title")
			{
               ...
               $nama_channel.= "  </ul>";
               $nama_channel.= "</div>";
               $nama_channel.="<hr></b><h3 class='page-title font-blue-steel font-lg bold' >".$aRow['title']."</h3>".shortNews( $aRow['news'], $wordCount = 30 )."...."."<hr> <a class='beritaProduk' href='#' id-produk='".$aRow['id_pnews']."' id-title='".$aRow['title']."' data-toggle='modal' data-target='#news-product'><button class='btn blue'> Read more <i class='m-icon-swapright m-icon-white'></i> </button> </a>";							
			   $row[] = $nama_channel;
			}
